...
class CurlHttpClient
{
	 ...
	 * @access public
	 */
	function send_post_data($url, $postdata, $ip=null, $timeout=120)
	{
	 ...
	 * @access public
	 */
	function fetch_url($url, $ip=null, $timeout=120)
	{
		$startTime = time();
		// set url to post to
		curl_setopt($this->ch, CURLOPT_URL,$url);
		...
		if($diff > 5) {
			//Log::error("fetching $url took $diff seconds");
		}
		return $retVal;
	}
	 ...
	 * @access public
	 */
	function fetch_into_file($url, $fp, $ip=null, $timeout=5000)
	{
		// set url to post to
		curl_setopt($this->ch, CURLOPT_URL,$url);
	 ...
	 * @access public
	 */
	function send_multipart_post_data($url, $postdata, $file_field_array=array(), $ip=null, $timeout=120)
	{
		//set various curl options first
